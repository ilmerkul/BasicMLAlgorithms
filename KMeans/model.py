...
class MyKMeans:
        ...
        for i in range(self.n_init):
            cluster_centers = np.array(
                [
                    [
                        np.random.uniform(
                            np.min(X[feature].values), np.max(X[feature].values)
                        )
            ...
            while j < self.max_iter:
                distances = np.apply_along_axis(
                    lambda cl: np.sum((X.values - cl) ** 2, axis=1).T,
                    1,
                    cluster_centers,
                )
                ...
                new_cluster_centers = np.array(
                    [
                        (
                            np.mean(X[labels == k].values, axis=0)
                            if len(X[labels == k])
    ...
    def predict(self, X: pd.DataFrame) -> np.array:
        ...
        distances = np.apply_along_axis(
            lambda cl: np.sum((X.values - cl) ** 2, axis=1).T, 1, self.cluster_centers_
        )
